...
class StrategyTrading(models.Model):
    ...
    strategy_type = fields.Selection([
        ("double_ema", "双均线策略"),
        ("sky_city", "天空之城策略")],
        default='double_ema', string="策略")
    entry_type = fields.Selection([
        ("double_ema", ""),]
        , default='double_ema', string="入场策略")
    # 公用参数
...
class StrategyTradingSub(models.Model):
    ...
    is_running = fields.Boolean("运行中")
    side = fields.Selection([("long", "做多"), ("short", "做空")], required=True, string="方向")
    entry_time = fields.Char("入场时间")
    entry_price = fields.Float("入场价格")
    exit_time = fields.Datetime("结束时间")
    exit_price = fields.Float("结束价格")
